...
「これは他のプロジェクトでも適用できるか?」で判断する。
- できる (LTM 設計原則、コードの一般則、UX 法則など) → `design/`
- できない (このプロジェクト固有の実装判断、命名規約、依存技術) → `project/<name>/`

実装名 (関数名 / スクリプト名 / プロジェクト名) が本文に出てくる時点で、それは `project/<name>/` に置く方が無難。
