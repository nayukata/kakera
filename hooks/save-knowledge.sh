...
STEP 2. 抽出候補を列挙
- transcript を読み、下記**抽出シグナル**に当たる箇所を**短文の見出しリスト**として 1 度に書き出す (この時点ではファイルを作らない)。シグナルに当たらない雑談 / 単なる作業ログは拾わない。
- これは「都度保存」ではない。session 全体を 1 度に俯瞰し、収束した結論 / 確立した判断だけを残す。途中で却下された案や言い直しの途中経過は拾わない。

抽出シグナル (観測可能な発話・会話の展開):
  - feedback: ユーザーが Claude の振る舞いを修正 / 承認した (「そうじゃない」「次はこうして」「それでいい」「今後はこう判断して」)
  - mistakes: 誤り / エラーの原因が判明し再発防止が言語化された (「またやった」「これが原因だった」「同じミス」)
  - decisions / design / project: 技術選定や設計方針を採用・確定した (「これでいく」「この方針で」「採用」)
  - user: ユーザーの背景 / 好み / 働き方が新たに分かった
  - 壁打ち到達点: 数往復の議論の末にユーザーが自分の言葉で結論 / 気づきに達した (議論の収束点)。単発の事実回答は除く

- 同時に **questions/ 候補** も列挙する。シグナル:
...
- questions 候補は解決済 / 未解決を**問わず**全部挙げる (探究履歴化)。
